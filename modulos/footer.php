		<div id="footer" class="row-fluid">
			<div class="offset1 span2">
				<ul class="noticias">
					<li>Account Manager</li>
					<li><a href="#">My Account</a></li>
					<li><a href="#">My Renewals</a></li>
					<li><a href="#">My Upgrades</a></li>
					<li><a href="#">Account Settings</a></li>
					<li><a href="#">Customer Information</a></li>
					<li><a href="#">Order History</a></li>
				</ul>
			</div>
			<div class="span2">
				<ul class="noticias">
					<li>Shopping</li>
					<li><a href="#">Offer Disclaimers</a></li>
					<li><a href="#">Domain Search</a></li>
					<li><a href="#">Product Catalog</a></li>
					<li><a href="#">Producr Advisor</a></li>
					<li><a href="#">Gift Cards</a></li>
					<li><a href="#">Mobile</a></li>
				</ul>
			</div>
			<div class="span2">
				<ul class="noticias">
					<li>Resources</li>
					<li><a href="#">Webmail</a></li>
					<li><a href="#">WHOIS search</a></li>
					<li><a href="#">ICANN Confirmation</a></li>
					<li><a href="#">Affiliates</a></li>
					<li><a href="#">Connect with Us</a></li>
					<li><a href="#">Gadgets / Widgets</a></li>
				</ul>
			</div>
			<div class="span2">
				<ul class="noticias">
					<li>Help and Support</li>
					<li><a href="#">Support &amp; Sales</a></li>
					<li><a href="#">Billing Support</a></li>
					<li><a href="#">Email Our Support</a></li>
					<li><a href="#">FAQ’s</a></li>
					<li><a href="#">User’s Guides</a></li>
					<li><a href="#">Report Spam</a></li>
				</ul>
			</div>
			<div class="span2">
				<ul>
					<li>About</li>
					<li><a href="#">Careers</a></li>
					<li><a href="#">Security Center</a></li>
					<li><a href="#">Company Info</a></li>
					<li><a href="#">News Center</a></li>
					<li><a href="#">Customer</a></li>
					<li><a href="#">What’s New</a></li>
				</ul>
			</div>
		</div>
		<footer class="row-fluid">
			<p>Todos los derechos reservados por <strong>Altivia<span>OT</span> © 2012.</strong> Copyright @ 2012.
			<br>Visita nuestro <a href="http://blog.altiviaot.com"><strong>blog</strong></a>.</p>
		</footer>
	</div>
   <body>
   <?php if (file_exists("../modulos/script.php")){ include('../modulos/script.php'); } ?>
</html>