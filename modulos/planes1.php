...
<?php
	$sql='SELECT `nombre`,`discoduro`,`transferencia`,`emails`,`ftp`,`precio` FROM `planes` WHERE `estado` = 1';
	...
	if (mysql_affected_rows() > 0) {
		while($row=mysql_fetch_array($result)){
			...
			echo '<a href="../plans/'.$row['nombre'].'.php" class="btn btn-inverse">Learn More</a></div>';
		}
	}
?>
</div>